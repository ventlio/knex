...
class Builder extends EventEmitter {
  ...
  insert(values, returning, options) {
    ...
    return this.insertInternal(values, returning, options);
  }

  batchInsert(rows, chunkSize = 1000) {
    const tableName = this.and._single.table;

    if (!Array.isArray(rows)) {
      throw new Error('Batch insert requires an array of objects');
    }
    ...
    return this.client
      .transaction((trx) => {
        const promises = chunks.map((chunk) => {
          return trx(tableName).insert(chunk);
        });
        return Promise.all(promises);
      })
      .then((results) => {
        // Flatten the results array and extract the inserted IDs
        return [].concat(...results);
      });
  }

  insertInternal(values, returning, options) {
    this._method = 'insert';
